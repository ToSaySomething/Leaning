...
MyActivity::MyActivity(const ServerSocket& socket, Params::Ptr pParams):
    ...
    _socket(socket)
{
    ...
    if (pParams)
    {//add
        int toAdd = MAXThreads /*pParams->getMaxThreads()*/ - pool.capacity();
...
void MyActivity::enqueue(const ThreadPool& pool, const StreamSocket& ss)
{
   queue.enqueueNotification(new WorkNotification(ss));
   static const std::string threadName("worke1");
   //pool.start(*this,threadName);
